...
type FirmataCommand uint8
type SysExCommand uint8
type SerialPort uint8

// Firmata related constants
const (
...
func (c FirmataCommand) String() string {
	...
	case c == EnableAnalogInput:
		return fmt.Sprintf("EnableAnalogInput (0x%x)", uint8(c))
	case c == EnableDigitalInput:
		return fmt.Sprintf("EnableDigitalInput (0x%x)", uint8(c))
	case c == SetPinMode:
		return fmt.Sprintf("SetPinMode (0x%x)", uint8(c))
	case c == ReportVersion:
		return fmt.Sprintf("ReportVersion (0x%x)", uint8(c))
	case c == SystemReset:
...
func (c SysExCommand) String() string {
	...
	case c == ReportFirmware:
		return fmt.Sprintf("ReportFirmware (0x%x)", uint8(c))
